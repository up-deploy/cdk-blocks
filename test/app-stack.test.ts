...
const s3 = (role: string, seq?: string) => ({
  block: "s3",
  role,
  seq,
  blockRef: "v0.3.0",
...
describe("one stack per app, holding many components", () => {
  ...
  const stack = new AppStack(app, "App", {
    ...
    components: [s3("docs"), s3("uploads"), s3("docs", "02")],
  });
  ...
  test("each component composes its own name from role and seq", () => {
    for (const name of ["up-s3-a231-docs-dev-01", "up-s3-a231-uploads-dev-01", "up-s3-a231-docs-dev-02"]) {
  ...
  test("POLICY: outputs are unique per component", () => {
    for (const id of ["Docs01BucketName", "Uploads01BucketName", "Docs02BucketName"]) {
  ...
  test("each bucket carries its own role tag", () => {
    template.hasResourceProperties("AWS::S3::Bucket", {
      BucketName: "up-s3-a231-uploads-dev-01",
...
describe("the component list is validated before anything is built", () => {
  ...
  // POLICY: (block, role, seq) is what guarantees a unique resource name, so a duplicate is
  // refused at parse time rather than surfacing as a duplicate-name error from AWS at deploy.
  test("POLICY: two components with the same block, role and seq are refused", () => {
    const raw = JSON.stringify([s3("docs"), s3("docs")]);
    expect(() => parseComponents(raw)).toThrow(/Duplicate component 's3\/docs\/01'/);
  });
